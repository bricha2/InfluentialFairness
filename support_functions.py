...
def calc_disp(model, attr, labels, sensitive):
...
def prepPredictor(model_type, X, y, X_train, y_train, random_state):
    ...
    if model_type == 'Kneighbors':
        ...
        search=GridSearchCV(estimator=KNR, param_grid=search_grid, scoring=make_scorer(RBO.bal_rbo),
                            n_jobs=1, cv=cv)
        search.fit(X,y)
        ...
        search=GridSearchCV(estimator=GBR, param_grid=search_grid, scoring=make_scorer(RBO.bal_rbo),
                            n_jobs=1, cv=cv)
        search.fit(X,y)
        ...
        search=GridSearchCV(estimator=RR, param_grid=search_grid, scoring=make_scorer(RBO.bal_rbo),
                            n_jobs=1, cv=cv)
        search.fit(X,y)
...
def evalPredictor(model, X_test, y_test):
    ...
    for k in range(1, minNum, 5):
        rbo_list.append(RBO.bal_rbo(y_test.iloc[0:k], pred[0:k]))
