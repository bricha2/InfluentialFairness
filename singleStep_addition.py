...
if __name__ == "__main__":
    ...
    for metric in metrics:
        ...
        for label, title, temp_df in zip(['BB_public', 'GB'],
                                         ['BB Estimations on Public','GB Predictor on Public'], 
                                         [BB_public, df]):
            
            fn = "%s/out/static_addition_%s_%s_%s_iterations.csv"%(data_name, data_name, metric, label)
            print("Addition: %s, %s, %s"%(data_name, metric, label))
            ...
            for i in range(20,len(short_df),20):
                ...
                mean_inf = temp_df['influences'].head(i).mean()
                sum_inf = sum_inf + sum(temp_df['influences'].head(i))
                temp = temp_df.head(i).drop(['influences'], axis=1)
            ...
            disp_tb.to_csv("%s/out/static_addition_%s_%s_%s_iterations.csv"%(data_name, data_name, metric, label))
