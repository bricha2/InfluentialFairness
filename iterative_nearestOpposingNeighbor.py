    ...
    for metric in metrics:
        ...
        for step_size in step_sizes:
            ...
            for name, title, inf_df in zip(['GT', 'BB_public', 'GB'],
                ...
                fn = "%s/out/nonstatic_nearestOppNeigh_%s_%s_%s_%s_iterations.csv"%(data_name, data_name, metric, 
                                                                                    name,str(step_size))
                ...
                while (n_points < original_size*2):
                    ...
                    temp_extended = topn_samples.assign(**{f'C{j}': topn_samples[outcome_column_name] for j in range(1, n+1)})
                    ...
                    temp = not_df_train.iloc[temp_extended.apply(query, axis = 1)]
                ...
                disp_tb.to_csv("%s/out/nonstatic_nearestOppNeigh_%s_%s_%s_%s_iterations.csv"%(data_name, data_name, metric, 
                                                                                    name,str(step_size)))
