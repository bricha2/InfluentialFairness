...
if __name__ == "__main__":
    ...
    for metric in metrics:
        ...
        GB_public = df_public.copy()
        GB_public[group_column_name] = [0 if x == default_minority_group else 1 for x in GB_public[group_column_name]]
        ...
        X_public = prepX(GB_public, 'train', outcome_column_name=outcome_column_name, withInf = False)

        # Apply influence predictor
        pred = ifs_predictor.predict(X_public)

        # Add to original df
        GB_public['influences'] = pred
        ...
        for step_size in step_sizes:
            ...
            for name, title, inf_df in zip(['BB_public', 'GB'],
                                             ['BB Estimations on Public','GB Predictor on Public'], 
                                             [BB_public, GB_public]):

                fn = "%s/out/nonstatic_addition_%s_%s_%s_%s_iterations.csv"%(data_name, data_name, metric, 
                                                                                          name,str(step_size))
                ...
                disp_tb.to_csv("%s/out/nonstatic_addition_%s_%s_%s_%s_iterations.csv"%(data_name, data_name, metric, 
                                                                                          name,str(step_size)))
