    ...
    for metric in metrics:
        ...
        for step_size in step_sizes:
            ...
            for name, title, inf_df in zip(['GT', 'BB_public', 'GB'],
                ...
                fn = "%s/out/nonstatic_revOpp_%s_%s_%s_%s_iterations.csv"%(data_name, data_name, metric, 
                                                                                    name,str(step_size))
                ...
                # Only remove until the size of df_train has doubled
                ...
                while (n_points < original_size*2):
                    ...
                    if step_size == 'knee':
                        #print("In knee step")
                        # Decreasing list after reversal
                        expandingMean = short_df['influences'].iloc[::-1].expanding().mean()
                        if len(expandingMean) == 1:
                            break
                        topn = getKnee(expandingMean, "convex", "decreasing", S=75)
                        if topn == None:
                            break
                    else:
                    ...
                    # grab the bottom n points
                    mean_inf = inf_df['influences'].tail(topn).mean()
                    topn_samples = inf_df.tail(topn).drop(['influences'], axis=1)

                    # Find the like points 
                    if name == 'GT' or 'train' in name:
                        points_matched = len(topn_samples)
                        opponents = not_df_train[not_df_train.index.isin(topn_samples.index)]
                        temp_train = not_df_train[~not_df_train.index.isin(topn_samples.index)]
                        ind_set = np.append(ind_set,topn_samples.index)
                        ind_set = np.unique(ind_set)
                    else:
                        opponents = not_df_train[not_df_train.apply(tuple,1).isin(topn_samples.apply(tuple,1))]
                        points_matched = opponents.shape[0]
                        temp_train = not_df_train[~not_df_train.apply(tuple,1).isin(topn_samples.apply(tuple,1))]
                        ind_set = np.append(ind_set,opponents.index)
                        ind_set = np.unique(ind_set)
                    
                    # Reverse the opponents and add
                    opponents[outcome_column_name] = [1 if x == 0 else 0 for x in opponents[outcome_column_name]]
                    temp_train = pd.concat([temp_train, opponents])
                    ...
                    elements_changed.append(len(ind_set))
                    ...
                    if 'GT' in name or 'GB' in name:
                        ...
                        if 'GB' in name:
                            ...
                            # Remove bottom n points from inf_df
                            inf_df = inf_df.tail(-topn).drop(['influences'], axis=1)
                        ...
                        influences = getBlackBoxScores(torch.from_numpy(X_test_min.astype(int)), torch.from_numpy(X_test.astype(int)), 
                ...
                d = {'N added':n_points_add,'Disparity':disparities, 'Accuracy':accuracies, 'Mean Influences':mean_infs, 'N changed':elements_changed}
                disp_tb = pd.DataFrame(d)
                disp_tb.to_csv("%s/out/nonstatic_revOpp_%s_%s_%s_%s_iterations.csv"%(data_name, data_name, metric, 
                                                                                    name,str(step_size)))
