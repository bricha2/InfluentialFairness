...
if __name__ == "__main__":
    ...
    for metric in metrics:
        ...
        for label, title, temp_df in zip(['GT', 'BB_public', 'GB'],
            ...
            fn = "%s/out/static_duplication_%s_%s_%s_iterations.csv"%(data_name, data_name, metric, label)
            print("Duplication: %s, %s, %s"%(data_name, metric, label))
            ...
            for i in range(20,len(short_df),20):
                ...
                temp = temp[temp.apply(tuple,1).isin(df_train.apply(tuple,1))]
            ...
            disp_tb.to_csv("%s/out/static_duplication_%s_%s_%s_iterations.csv"%(data_name, data_name, metric, label))
